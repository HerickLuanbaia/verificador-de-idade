...
function verificarIdade(){
    ...
    if(isNaN(idade) || idade < 0){
        ...
        menssagem = 'Você tá na flor da idade';
    }else if(idade < 60){
        menssagem = 'Você é CLT';
    }else{
        menssagem = 'Você é um espirito';
    }
...
class Particula{
    ...
    atualizar(){
        // Inverte a direção se a partícula atingir a borda da tela
    ...
    for(let a = 0; a < particulasArray.length; a++){
        for(let b = 0; b < particulasArray.length; b++){
            ...
            if(distancia < (canvas.width / 7) * (canvas.height / 7)){
                ctx.strokeStyle = `rgba(114, 0, 0, 1), ${1 - (distancia/20000)})`;
                ctx.lineWidth = 1;
                ctx.beginPath();
...
//Garante que o o mouse sai da área de efeito quando ele sai da janela 
